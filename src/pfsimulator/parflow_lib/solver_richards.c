...
typedef struct {
  ...
  int efdc;               /* 3D Hydrodynamic model for Lake areas - Added by Tomas Carlotto */
...
#ifdef HAVE_EFDC                /* VARIABLES FOR EFDC ONLY */

int efdc_def_procs;             /* EFDC processors number deffinition */
                ...
                Vector ** porosity_out, Vector ** saturation_out)
{
  ...
  MPI_Fint efdc_commWorld, efdc_intercomm;
  MPI_Fint efdc_nproc;
  
  #define PATH_MAXN 100
	char cwd[PATH_MAXN];
	getcwd(cwd, sizeof(cwd));	
	strcat(cwd, "/hyd");
  
  switch (public_xtra->efdc)
        {
          case 0:
          {
            // No EFDC model
          ...
          case 1:
          {
			  char *path_exe = getenv("PARFLOW_DIR");
			  char *dir = "/efdc/";
			  size_t len = strlen(path_exe) + strlen(dir) + 1;
			  char* filepath = malloc(len);	
						
			  strcpy(filepath, path_exe);
			  strcat(filepath, dir);
									
			  ////MPI_Info info;
			  ////MPI_Info_create(&info);
			  
			  //MPI_Fint efdc_commWorld, efdc_intercomm, efdc_info;
			  
			  //MPI_Fint efdc_nproc;					
								
								strcat(filepath, "EFDC");			
								char ch_cell;			
								FILE *copy_EFDC_cell, *paste_EFDC_celllt;
								char *dirfile_cell = filepath;			
								copy_EFDC_cell = fopen(dirfile_cell, "rb");				
								const char dirfile_paste_celllt[100] = "hyd/EFDC";
								paste_EFDC_celllt = fopen(dirfile_paste_celllt, "wb");			
								...
								//MPI_Info_set(info,"wdir","hyd");		
								
								//efdc_info = MPI_Info_c2f(info);
								
								#define BUFF_SIZE 4097
								unsigned char ch[BUFF_SIZE];			
								int x;
								while (!feof(copy_EFDC_cell)) {
								   x = fread(ch,1,BUFF_SIZE, copy_EFDC_cell);			   
									 if(fwrite(ch, 1, x, paste_EFDC_celllt) != x){
										// display error
										break;
									 }				 
								}			
								...
								struct stat tmp;
								stat(filepath, &tmp);
								chmod("hyd/EFDC", tmp.st_mode);
								
			  efdc_nproc = (public_xtra->efdc_def_procs);
          ...
          default:
          {
            amps_Printf("Calling unknown EFDC model");
          }
        }   
    ...
    switch (public_xtra->efdc)
        {
          ...
          case 1:
          {
			...
			MPI_Info_set(info,"wdir",filepath);	
			
			//MPI_Info_set(info,"wdir","/home/labhidro/parflow/install/bin/");	    		
			...
			*/
			...
			efdc_intercomm = MPI_Comm_c2f(intercomm);
			efdc_commWorld = MPI_Comm_c2f(MPI_COMM_WORLD);
			...
			/*		
					...
					//MPI_Info_set(info,"wdir","/home/labhidro/parflow/testeGrass/testeBCPressurePFPL10m/hyd");	
					MPI_Info_set(info,"wdir","hyd");		
			...
			*/
			...
			//MPI_Comm_spawn(command, MPI_ARGV_NULL, 6, info, 0, MPI_COMM_WORLD, &intercomm, MPI_ERRCODES_IGNORE);
			  
			//MPI_Fint efdc_nproc;			
			
			//efdc_nproc = (public_xtra->efdc_def_procs);
			
			PF_CALL_EFDC(&efdc_nproc,&efdc_info,&efdc_commWorld,&efdc_intercomm);
...
SolverRichardsNewPublicXtra(char *name)
{
  ...
  NameArray hydrodynamic_switch_na;                 // Added by Tomas Carlotto
  ...
  hydrodynamic_switch_na = NA_NewNameArray("none EFDC");
  sprintf(key, "%s.HYDRODYNAMIC", name);
  switch_name = GetStringDefault(key, "none");
  switch_value = NA_NameToIndex(hydrodynamic_switch_na, switch_name);
  switch (switch_value)
  {
    case 0:
    {
...
#else
      InputError
        ("Error: <%s> used for key <%s> but this version of Parflow is compiled without EFDC\n",
        switch_name, key);
#endif
      break;
    }
  ...
  NA_FreeNameArray(hydrodynamic_switch_na);
  //=======
  
  /* Following are only used /w CLM - Added by Tomas Carlotto*/
#ifdef HAVE_EFDC
  sprintf(key, "%s.EFDC.EFDCNproc", name);
  public_xtra->efdc_def_procs = GetIntDefault(key, 1);
