...
C  
     ...
     &        +0.5*HP(L)*(Z(K)+Z(K-1)) )*B(L,K)  
      ...
      DO K=1,KC  
        DO LL=1,NCBW  
          L=LCBW(LL)  
          VOLOUT=VOLOUT-DELT*UHDY2(LEAST(L),K)*DZC(K)  
          WVOLOUT=WVOLOUT-DELT*UHDY2(LEAST(L),K)*DZC(K)
          SALOUT=SALOUT-DELT*MIN(UHDY2(LEAST(L),K),0.)*SAL1(LEAST(L),K)*DZC(K)  
     &        -DELT*MAX(UHDY2(LEAST(L),K),0.)*SAL1(L,K)*DZC(K)  
          DYEOUT=DYEOUT-DELT*MIN(UHDY2(LEAST(L),K),0.)*DYE1(LEAST(L),K)*DZC(K)  
     &        -DELT*MAX(UHDY2(LEAST(L),K),0.)*DYE1(L,K)*DZC(K)  
          DO NS=1,NSED  
            SEDOUT2T(NS)=SEDOUT2T(NS)  
     &          -DELT*MIN(UHDY2(LEAST(L),K),0.)*SED(LEAST(L),K,NS)*DZC(K)  
     &          -DELT*MAX(UHDY2(LEAST(L),K),0.)*SED(L,K,NS)*DZC(K)  
          ENDDO  
          DO NS=1,NSND  
            SNDOUT2T(NS)=SNDOUT2T(NS)  
     &          -DELT*MIN(UHDY2(LEAST(L),K),0.0)*SND(LEAST(L),K,NS)*DZC(K)  
     &          -DELT*MAX(UHDY2(LEAST(L),K),0.0)*SND(L,K,NS)*DZC(K)  
          ENDDO  
          DO NT=1,NTOX  
            TOXOUT2T(NT)=TOXOUT2T(NT)  
     &          -DELT*MIN(UHDY2(LEAST(L),K),0.)*TOX(LEAST(L),K,NT)*DZC(K)  
     &          -DELT*MAX(UHDY2(LEAST(L),K),0.)*TOX(L,K,NT)*DZC(K)  
          ENDDO  
          PPEOUT=PPEOUT-DELT*UHDY2(LEAST(L),K)*G*DZC(K)*(0.5*(BELV(L)+
     &       BELV(LEAST(L)))+0.125*(HP(L)+H2P(L)+HP(LEAST(L))+H2P(LEAST(L)))*(Z(K)+
     &        Z(K-1)))  
          BBEOUT=BBEOUT-DELT*MIN(UHDY2(LEAST(L),K),0.)*DZC(K)*GP*( BELV(LEAST(L))
     &        +0.5*HP(LEAST(L))*(Z(K)+Z(K-1)) )*B1(LEAST(L),K)  
     &        -DELT*MAX(UHDY2(LEAST(L),K),0.)*DZC(K)*GP*( BELV(L)  
     ...
     &        +DELT*MAX(UHDY2(L,K),0.)*SAL(LWEST(L),K)*DZC(K)  
          DYEOUT=DYEOUT+DELT*MIN(UHDY2(L,K),0.)*DYE(L,K)*DZC(K)  
     &        +DELT*MAX(UHDY2(L,K),0.)*DYE(LWEST(L),K)*DZC(K)  
          DO NS=1,NSED  
            SEDOUT2T(NS)=SEDOUT2T(NS)  
     &          +DELT*MIN(UHDY2(L,K),0.)*SED(L,K,NS)*DZC(K)  
     &          +DELT*MAX(UHDY2(L,K),0.)*SED(LWEST(L),K,NS)*DZC(K)  
          ENDDO  
          DO NS=1,NSND  
            SNDOUT2T(NS)=SNDOUT2T(NS)  
     &          +DELT*MIN(UHDY2(L,K),0.)*SND(L,K,NS)*DZC(K)  
     &          +DELT*MAX(UHDY2(L,K),0.)*SND(LWEST(L),K,NS)*DZC(K)  
          ENDDO  
          DO NT=1,NTOX  
            TOXOUT2T(NT)=TOXOUT2T(NT)  
     &          +DELT*MIN(UHDY2(L,K),0.)*TOX(L,K,NT)*DZC(K)  
     &          +DELT*MAX(UHDY2(L,K),0.)*TOX(LWEST(L),K,NT)*DZC(K)  
          ENDDO  
         PPEOUT=PPEOUT+DELT*UHDY2(L,K)*G*DZC(K)*(0.5*(BELV(L)+BELV(LWEST(L)))
     &        +0.125*(HP(L)+H1P(L)+HP(LWEST(L))+H1P(LWEST(L)))*(Z(K)+Z(K-1)) )  
          BBEOUT=BBEOUT+DELT*MIN(UHDY2(L,K),0.)*DZC(K)*GP*(BELV(L)  
     &        +0.5*HP(L)*(Z(K)+Z(K-1)) )*B(L,K)  
     &        +DELT*MAX(UHDY2(L,K),0.)*DZC(K)*GP*(BELV(LWEST(L))  
     &        +0.5*HP(LWEST(L))*(Z(K)+Z(K-1)) )*B(LWEST(L),K)  
        ENDDO  
      ENDDO  
      DO K=1,KC  
        DO LL=1,NCBN  
